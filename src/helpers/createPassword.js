...
const LOWER_CASE_CODE = String.fromCharCode(...[...Array(122 - 97 + 1).keys()].map(i => i + 97)) + String.fromCharCode(241);
const UPPER_CASE_CODE = String.fromCharCode(...[...Array(90 - 65 + 1).keys()].map(i => i + 65)) + String.fromCharCode(209);
const NUMBERS_CODE = String.fromCharCode(...[...Array(57 - 48 + 1).keys()].map(i => i + 48));
const SYMBOLS_CODE = String.fromCharCode(...[...Array(47 - 33 + 1).keys()].map(i => i + 33))
...
export function createPassword(length, upperCase = false, numbers = false, symbols = false) {
    let result = "";
    let characters = LOWER_CASE_CODE;
